...
Webhooks allow you to get notified about events happening in your Plain workspace (_Plain events_). You can react to these events in many ways, such as:
...
If you want you can include authentication credentials in the url (`https://username:password@example.com`) which will then be sent along the webhook request in an `Authorization` header:
...
| `payload`         | `object` | The Plain event's payload [(Example)](/webhooks/events/customer-status-transitioned);                    |
